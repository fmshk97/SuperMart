...
namespace Assignment.Apis.Controllers
{
    ...
    public class StoresController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("{storeName}/products")]
        public async Task<IActionResult> AddProductToStoreAsync([FromRoute] string storeName, [FromBody] ProductInputModel product)
        {
            if (string.IsNullOrWhiteSpace(storeName))
            {
                var errorMessage = "Invalid store name provided.";
            ...
            if (product == null || string.IsNullOrWhiteSpace(product.Name) || string.IsNullOrWhiteSpace(product.Category))
            {
            ...
            var productDal = product.ToDalEntity();
            await _productsRepository.AddProductAsync(storeName, productDal);
            _logger.LogInformation($"Product '{product.Name}' successfully added to the store '{storeName}'.");

            return Ok($"Product '{product.Name}' added Successfully to the store '{storeName}'.");
        }
